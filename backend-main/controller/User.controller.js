...
const getNextBlock = async function () {
    const lastBlock = await BlockchainModal.findOne({}, {}, { sort: { heigh: -1 } }).exec();
    const heigh = lastBlock ? lastBlock.heigh : -1; // Establecer el número de orden anterior o 122 si no hay ningún carrito
        ...
        if (email) {
            ...
            if (!email.respondido) {
               ...
               //console.log("procesando ....")
                ...
                const tieneDocumentos = await BlockchainModel.findOne() !== null;
                ...
                if (usuario) {
                   ...
                   // console.log("tieneDocumentos", tieneDocumentos)
                    ...
                    if (!tieneDocumentos) {
                        ...
                        const altura = await getNextBlock()
                       ...
                       // console.log("altura", altura)
                        ...
                        const bloqueAnterior = await BlockchainModel.findOne({ heigh: altura - 1 })
                        const ultimoBloqueEmpresa = await BlockchainModel.findOne({ enterpriseId: idEmpresa })
                            .sort({ heigh: -1 })
                            .exec();
                        //console.log("esta es el array", bloqueAnteriorEmpresaArray)
                        let hashEmpresaAnterior;
                        ...
                        if (ultimoBloqueEmpresa) {
                            // console.log("entre", bloqueAnteriorEmpresaArray.length)
                            //console.log("bloque indivudal", bloqueAnteriorEmpresa)
                            // hashEmpresaAnterior = null
                            bloqueAnteriorEmpresa = ultimoBloqueEmpresa
                            hashMainAnterior = bloqueAnterior.hashMain
                            heighEnterprise = ultimoBloqueEmpresa.heigh + 1 
                            //console.log("esta es la altura de heighENterprise", heighEnterprise)
                            hashEmpresaAnterior = ultimoBloqueEmpresa.hashEnterprise
                ...
                const fin = performance.now(); // Registra el tiempo de finalización

                const tiempoTranscurrido = fin - inicio;

                console.log(`Tiempo de ejecución: ${tiempoTranscurrido} milisegundos para crear`);
...
UserCtrl.updateTreatmente = async (req, res) => {
    ...
    if (!consent) {
        ...
        if (treatmentsToEliminate) {
            ...
            const bloqueAnterior = await BlockchainModel.findOne({ heigh: altura })
            const bloqueAnteriorEmpresaArray = await BlockchainModel.find({ enterpriseId: idEmpresa })
            //console.log("esta es el array", bloqueAnteriorEmpresaArray)
            let hashEmpresaAnterior;
            let hashMainAnterior;
...
UserCtrl.deleteConsent = async (req, res) => {
    ...
    if (!consent) {
        ...
        const bloqueAnterior = await BlockchainModel.findOne({ heigh: altura - 1 })
        //console.log("bloqueAnterioor", bloqueAnterior)
        const ultimoBloqueEmpresa = await BlockchainModel.findOne({ enterpriseId: idEmpresa })
        .sort({ heigh: -1 })
        .exec();
        //console.log("esta es el array", bloqueAnteriorEmpresaArray)
        ...
        console.log(`Tiempo de ejecución: ${tiempoTranscurrido} milisegundos para eliminar`);
...
UserCtrl.updateData = async (req, res) => {
    ...
    if (!user) {
        ...
        if (!consent) {
            ...
            console.log(`Tiempo de ejecución: ${tiempoTranscurrido} milisegundos para editar`);
